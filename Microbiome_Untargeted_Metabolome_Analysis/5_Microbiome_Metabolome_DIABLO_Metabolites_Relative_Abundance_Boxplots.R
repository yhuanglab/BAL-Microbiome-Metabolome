...
number_of_otus_to_plot <- 10 #based on inflection point from stripchart if there is one; otherwise plot top 10 Metabolites
response_variable <- target_variable #variable whose levels against which we plot Metabolite relative abundance
...
#DO NOT MODIFY BELOW#
#*******************#
env <- read.csv("~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_Colorado_Stratified/Spiromics_No_Severe_No_Healthy/Data/Clinical_Parameters_04_21_20_No_Severe_No_Healthy.csv")
bac <- read.csv('~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_Colorado_Stratified/Spiromics_No_Severe_No_Healthy/Data/BALF_C18_Norm_Log2_Transposed_No_Severe_No_Healthy.csv') #Metabolite relative abundance data
#ids <- read.csv("~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_Colorado_Stratified/Spiromics_No_Severe_No_Healthy/Data/Sample_IDs_C18_Pos_BALF.csv", header=T)

df <- read.csv(paste("~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_Colorado_Stratified/Spiromics_No_Severe_No_Healthy/Output/DIABLO/", target_variable, "/DIABLO_Model_Features_ORs_P_Values.csv", sep = ""), stringsAsFactors = FALSE) #table with Metabolites, loadings, and number of models in which they were selected
...
#######################################################################################
#Analysis of each group for significance of differences in all Metabolite relative abundances#
#######################################################################################
...
for (i in 1:(ncol(bac_all)-1)) { #run on all Metabolites except the last column, which is k-means cluster
...
p_value_list_corrected <- p.adjust(p_value_list_corrected, method = "BH") #adjust p-values for FDR across all Metabolites
...
colnames(df_raw) <- c(paste("Intensity in", response_variable_levels[[1]]),
				  paste("Intensity in", response_variable_levels[[2]]),
				  "Uncorrected_P-Value", 
				  "BH-Adjusted_T-Test_P-Value")
write.csv(df_raw, file = paste("~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_Colorado_Stratified/Spiromics_No_Severe_No_Healthy/Output/DIABLO/", target_variable, "/Metabolite_Significance_Tests_by_Levels_of_", target_variable, ".csv", sep = ""))

#**************#
#SUBSET OF MetaboliteS#
#**************#

bac2 <- bac[, which(colnames(bac) %in% bac_list$"Feature")] #subset to those Metabolites selected by the most models
...
df <- df_raw[which(rownames(df_raw) %in% bac_list$"Feature"), ] #subset to those Metabolites selected by the most models
...
myplot <- function(i) { #plots individual boxplots using ggplot
	...
	#Generate boxplot
	pdf(paste("~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_Colorado_Stratified/Spiromics_No_Severe_No_Healthy/Output/DIABLO/", target_variable, "/Metabolite_Boxplots/", i, "_Intensity_by", target_variable, ".pdf", sep=""))
	p <- ggplot(data = dz) +
		...
		labs(x = "Level", y = "Intensity") +
		ggtitle(str_wrap(paste(i, "Intensity by Level of", response_variable, response_variable_levels_label, sep=" "), width = 40)) + #plot title
...
pdf(paste("~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_Colorado_Stratified/Spiromics_No_Severe_No_Healthy/Output/DIABLO/", target_variable, "/Metabolite_Boxplots/All_Metabolites_Intensity_.pdf", sep=""), width = 20, height = 20)
