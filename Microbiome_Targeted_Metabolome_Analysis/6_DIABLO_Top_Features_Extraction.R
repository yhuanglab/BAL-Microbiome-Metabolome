...
if (model_sig[11, "Mean.Value.Over.100.Resampling.Iterations"] > 0.05) {
	...
	#top_features <- read.csv('~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_UNC_Stratified/Spiromics_No_Severe_No_Healthy/Output/DIABLO/FVC_BDRESPONSE_PCT_ctb_1_12above/DIABLO_Model_Features_ORs_P_Values.csv')
	...
	labels <- c("Group", "Phenotype", "Phenotype_Figure_Name", "Feature", "Number_of_Models", "Median_Log_OR_In_Models_Where_Feature_Significant") #column names for final data table
	...
	if (target_variable == "BRON_PEX_HCUDRUG_365_A.y_1_Oneormore_0_None") {
		...
		write.table(df, file = "~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_UNC_Stratified/Spiromics_No_Severe_No_Healthy/Output/Heatmap/DIABLO_Num_Models_Heatmap/Demographic_Features_by_Num_Sig_Models.csv", 
						sep = ",", 
						append = TRUE, 
						row.names = FALSE)
	} else {
	...
	#	features_above_cut_point <- first_drop - 1
	#}
	labels <- c("Group", "Phenotype", "Phenotype_Figure_Name", "Feature", "Number_of_Models", "Median_Log_OR_In_Models_Where_Feature_Selected") #column names for final data table
	...
	if (is.null(top_features_otus)) {
		...
		if (target_variable == "BRON_PEX_HCUDRUG_365_A.y_1_Oneormore_0_None") { #write column names in case first model has no features
			...
			write.table(df, file = "~/Desktop/Spiromics_BAL_Microbiome_Metabolome_V2/Spiromics_UNC_Stratified/Spiromics_No_Severe_No_Healthy/Output/Heatmap/DIABLO_Num_Models_Heatmap/Top_Features_by_Num_Sig_Models_No_Unknowns.csv", 
							sep = ",", 
							append = TRUE, 
							row.names = FALSE, 
							col.names = FALSE)
